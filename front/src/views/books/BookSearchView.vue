  ...
  <main class="container py-4">
    ...
    <div class="row">
      ...
      <aside class="col-md-2 mb-4">
        <div class="filter-title fw-bold mb-2">카테고리</div>
        ...
        <div v-else>
          ...
          <div v-if="viewType === 'list'">
            <div v-for="book in filteredBooks" :key="book.book_ISBN13"
              class="d-flex align-items-start mb-4 pb-3 border-bottom book-list-elem" @click="onClickCard(book.pk)">
              ...
              <div class="flex-grow-1">
                <div class="book-title fw-bold">{{ book.book_title }}</div>
                <div class="small mb-1">{{ book.book_publisher }}</div>
                <div class="small mb-1">{{ book.book_pub_date }}</div>
                <div class="small mb-2">{{ book.book_description.slice(0, 40) }}...</div>
          ...
          <!-- 그리드형 보기: viewType === 'grid'일 때 카드형으로 배치 -->
          <div v-else class="row row-cols-1 row-cols-md-3 g-4">
            <div v-for="book in filteredBooks" :key="book.book_ISBN13" class="col book-grid-elem" @click="onClickCard(book.pk)">
              <div class="card h-100">
                <img :src="bookStore.API_URL + book.book_cover_img" class="card-img-top" alt="cover"
                  style="height: 180px; object-fit: cover;">
                <div class="card-body">
                  <h5 class="card-title">{{ book.book_title }}</h5>
                  <h6 class="card-subtitle mb-2">{{ book.book_publisher }}</h6>
...
.book-list-elem:hover, .book-grid-elem:hover {
  /* box-shadow: 0 8px 32px 0 #a1a1a1; */
  box-shadow: 0 4px 24px 0 #ccc;
}
